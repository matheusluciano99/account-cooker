# account-cooker verification record
...
```bash
cargo run --release --quiet --bin account-cooker -- \
  demo --agents 12 --days 3 --seed 1
```

Headline output:

| result | naive | account-cooker | legacy |
|---|---:|---:|---:|
...
The unrounded account-cooker report, generated by `dump` followed by `report`, was:
...
same-timestamp co-activity (the exact-ts adversary); only hardened account-cooker resists both.

Raw F1:

| seed | naive | account-cooker | legacy |
|---:|---:|---:|---:|
| 1 | 1.000 | 0.246 | 1.000 |
| 2 | 1.000 | 0.272 | 0.957 |
| 3 | 1.000 | 0.264 | 1.000 |
...
Naive and legacy are re-identified at AUC 1.00; hardened account-cooker is driven to AUC 0.63
...
```bash
cargo run --release --quiet --bin account-cooker -- \
  cost --ledger /tmp/account-cooker-seed1-current.json \
...
Naive is fully de-anonymized (F1 1.00) by fee-payer linkage at any scale; hardened account-cooker
...
```bash
cargo test -p account-cooker-cli --release \
...
```bash
cargo run --bin account-cooker -- personas \
  --out-dir /tmp/account-cooker-personas-proof
cargo run --release --bin account-cooker -- benchmark \
  --agents 3 --days 1 --seeds 1 \
  --personas-dir /tmp/account-cooker-personas-proof
```
...
```bash
COOKER_RPC_URL=http://127.0.0.1:8899 bash scripts/localnet-live-proof.sh
...
```bash
cargo run --features live --bin account-cooker -- live-transfer \
...
`solana confirm -v` on the memo tx shows `Program log: Memo (len 24): "account-cooker-live-proof-8f3a"`.
